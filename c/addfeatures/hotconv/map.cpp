...
#define MAKE_UNI_GLYPH(uv, gname) snprintf(gname, sizeof(gname), "uni%04X", uv)
...
typedef struct {
    ...
    const char *name;      /* Descriptive name (debugging purposes only) */
} UnicodeBlock;
...
typedef struct {
    ...
    const char *gn1;      /* ..and its AGL glyph name (NULL if not in AGL).   */
                          /* It is needed here since there is no reverse AGL  */
                          /* lookup; i.e. UV->gname.                          */
    UV_BMP uv2;           /* Heuristic char 2 (optional; UV_UNDEF if none)    */
    const char *gn2;      /* ..and its AGL glyph name (NULL if not in AGL)    */
...
void mapNew(hotCtx g) {
    mapCtx h = (mapCtx)MEM_NEW(g, sizeof(struct mapCtx_));
...
static int CTL_CDECL cmpCID(const void *first, const void *second) {
...
#if 0
static int CTL_CDECL cmpUnrecGlyphName(const void *first, const void *second) {
    ...
    if ((a->flags & GNAME_UNREC) && (b->flags & GNAME_UNREC)) {
        if ((a->flags & GNAME_UNREC_HAS_SUPP) &&
            ...
            return strcmp(a->gname.c_str(), b->gname.c_str());
...
static int CTL_CDECL cmpGlyphName(const void *a, const void *b) {
    return strcmp((*(hotGlyphInfo **)a)->gname.c_str(), (*(hotGlyphInfo **)b)->gname.c_str());
}

static int CTL_CDECL matchGlyphName(const void *key, const void *value) {
    return strcmp((char *)key, (*(hotGlyphInfo **)value)->gname.c_str());
}
...
hotGlyphInfo *mapName2Glyph(hotCtx g, const char *gname, const char **useAliasDB) {
    ...
    if (useAliasDB != NULL) {
        *useAliasDB = g->goadb->getFinalGlyphName(gname);
...
void mapGID2Name(hotCtx g, GID gid, char *msg, int msglen) {
    auto &hGID = g->glyphs[gid];

    if (hGID.srcName.empty()) {
        snprintf(msg, msglen, "%s", hGID.gname.c_str());
    } else {
        if (g->convertFlags & HOT_CONVERT_FINAL_NAMES)
            snprintf(msg, msglen, "%s", hGID.gname.c_str());
        else
            snprintf(msg, msglen, "%s", hGID.srcName.c_str());
    }
}
...
static int mapName2UVOverrideName(hotCtx g, const char *gname,
    ...
    if (uvOverrideName != NULL) {
        *uvOverrideName = uvName;
    }

    return (uvName != NULL);
}

static int CTL_CDECL cmpUV(const void *first, const void *second) {
...
hotGlyphInfo *mapUV2Glyph(hotCtx g, UV uv) {
    ...
    if (found != NULL) {
        ...
        for (i = 0; i < h->sort.glyphAddlUV.cnt; i++) {
            hotGlyphInfo *gi = &g->glyphs[h->sort.glyphAddlUV.array[i]];

            for (auto auv : gi->addlUV) {
                if (auv == uv) {
                    return gi;
                }
            }
...
static int CTL_CDECL matchBlock(const void *key, const void *value) {
    UV uv = *(UV *)key;
...
static void CTL_CDECL CMapMsg(mapCtx h, int msgType, const char *fmt, ...) {
    va_list ap;
    char msgVar[1024];
    char msg[2048];
...
#define IS_UNSET(var) ((var) != 0 && (var) != 1)
    ...
    const char *rangeName = codespace ? "codespace" : notdef ? "notdef" : charrange ? "cidchar" : "cid";
...
static int CTL_CDECL cmpRanges(const void *first, const void *second) {
    Range *a = (Range *)first;
...
/* sort function  */
static int CTL_CDECL cmpUVSEntry(const void *first, const void *second, void *cts) {
...
static void gnameError(hotCtx g, const char *message, const char *token, const char *filename, long line) {
    ...
    for (lineno = 1; g->cb.uvsGetLine(g->cb.ctx, buf, &lineLen) != NULL; lineno++) {
        ...
        *p++ = '\0';
        ...
        if (0 == strncmp(glyphTag, "CID+", 4)) {
            ...
            uvsEntry->cid = -1;
            STRCPY_S(uvsEntry->gName, sizeof(uvsEntry->gName), glyphTag);
        }
...
static int CTL_CDECL cmpGlyphNameUC(const void *key, const void *value) {
    return strcmp((char *)key, ((UnicodeChar *)value)->glyphName);
}

static UnicodeChar *getUVFromAGL(hotCtx g, const char *glyphName, int fatalErr) {
    UnicodeChar *found =
...
static int checkUniGName(hotCtx g, const char *gn, uint32_t *usv) {
#define IS_HEX(h) (isxdigit(h) && !islower(h))
    uint32_t code;
    const char *pHexStart;
    const char *p;
    long numHexDig;
    int uniName = 0;
...
/* Does any base component of gname represent a supplementary UV? e.g.
   ...
   0. */
static int gnameHasSuppUV(hotCtx g, const char *gname) {
    char *p;
...
static void assignUVsZapfDingbats(hotCtx g) {
    ...
    for (i = 1 /* Skip .notdef */; i < g->glyphs.size(); i++) {
        ...
        gi = &g->glyphs[i];
        found = (UnicodeChar *)bsearch(gi->gname.c_str(), zapfDingbatsList,
...
static void assignUVs(hotCtx g) {
    ...
    if (g->font.FontName == "ZapfDingbats") {
    ...
    for (i = 1 /* Skip .notdef */; i < g->glyphs.size(); i++) {
        ...
        if (checkUniGName(g, gi->gname.c_str(), &uv)) {
            /* --- Valid uni<CODE> or u<CODE> glyph name */
            addUVToGlyph(g, gi, uv);
            gi->flags |= GNAME_UNI;
        } else if (mapName2UVOverrideName(g, gi->gname.c_str(), &uvOverrideName) != 0) {
            std::string uvo = uvOverrideName;
            char *uvoc = (char *) uvo.data();  // XXX
            char *uvName = getNextUVName(&uvoc);
            while (uvName != NULL) {
                ...
                uvName = getNextUVName(&uvoc);
            }
        } else {
            UnicodeChar *uc = getUVFromAGL(g, gi->gname.c_str(), 0);

            if (uc != NULL &&
                gi->gname[strlen(gi->gname.c_str()) - 1] != AGL2UV_DBLMAP_CHAR) {
                /* --- present in AGL */
                char dblMap[128];
                char uni[8];
                int assignedUVs = 0;
                ...
                if (g->font.flags & HOT_DOUBLE_MAP_GLYPHS) {
                    snprintf(dblMap, sizeof(dblMap), "%s%c", gi->gname.c_str(), AGL2UV_DBLMAP_CHAR);
                ...
                if (gnameHasSuppUV(g, gi->gname.c_str())) {
        ...
        for (j = 255; j >= 0; j--) {
            ...
            if (hotEnc) {
                ...
                if (h->sort.platEnc[j] != NULL &&
                    h->sort.platEnc[j] != &g->glyphs[0]) {
                    gname = h->sort.platEnc[j]->gname.c_str();
                } else {
                    continue;
                }
            } else {
...
static void createMaccmap(hotCtx g, long script, long language,
                          hotEncoding *hotEnc, const UV_BMP *uvEnc,
                          int usePlatEnc) {
    int i;
    GID gid;
    int macCPInx = -2;
...
static void makeUnicodecmaps(hotCtx g) {
    ...
    for (i = 0; i < h->sort.uv.cnt; i++) {
        ...
        for (auto uv : gi->addlUV) {
            if (!IS_SUPP_UV(uv)) {
                cmapAddMapping(g, uv, GET_GID(gi), 2);
            }
        }
    }
    ...
    for (i = 0; i < h->sort.uv.cnt; i++) {
        ...
        for (auto uv : gi->addlUV) {
            cmapAddMapping(g, uv, GET_GID(gi), 4);
        }
    }
    if (cmapEndEncoding(g)) {
...
static void makeUVScmap(hotCtx g) {
    ...
    for (i = 0; i < h->uvs.entries.cnt; i++) {
        ...
        if (IS_CID(g)) {
            ...
            if (finalName != NULL) {
                STRCPY_S(uve->gName, sizeof(uve->gName), finalName);
            }
        }
...
static int CTL_CDECL cmpAFMOrder(const void *first, const void *second) {
    ...
    if (a != AFM_UNENC && b == AFM_UNENC) {
        ...
        return strcmp(((AFMChar *)first)->gi->gname.c_str(),
                      ((AFMChar *)second)->gi->gname.c_str());
    }
}
...
static void AFMPrintCharMetrics(hotCtx g) {
    ...
    if (IS_CID(g)) {
        ...
        for (i = 1; i < nGlyphs; i++) {
            ...
            next->code = (gi->code == ABF_GLYPH_UNENC) ? AFM_UNENC : gi->code;
            next->gi = gi;
...
static int CTL_CDECL cmpKernNamePair(const void *first, const void *second) {
...
static void AFMPrintKernData(hotCtx g) {
    ...
    for (i = 0; i < nPairs; i++) {
        ...
        kern = dnaNEXT(pairs);
        kern->first = g->glyphs[first].gname.c_str();
        kern->second = g->glyphs[second].gname.c_str();
...
void mapPrintAFM(hotCtx g) {
    ...
    printf("FontName %s\n", g->font.FontName.c_str());
    printf("FullName %s\n", g->font.FullName.c_str());
    printf("FamilyName %s\n", g->font.FamilyName.c_str());
...
#define SUBSET_MAX 1000 /* Heuristic for printing */
    ...
    if (IS_CID(h->g)) {
        ...
        for (i = 0; i < nGlyphs; i++) {
            ...
            if (nGlyphs <= SUBSET_MAX || !gi->addlUV.empty()) {
    ...
    for (i = 0; i < (uint32_t)h->sort.gname.cnt; i++) {
        ...
        fprintf(stderr, " %-20s", gi->gname.c_str());
...
static void CTL_CDECL dbgUse(int arg, ...) {
...
static void initGlyphs(hotCtx g) {
    ...
    for (i = 0; i < nGlyphs; i++) {
        ...
        if (!IS_CID(g)) {
            if (gi->code != ABF_GLYPH_UNENC) {
                ...
                for (auto code : gi->sup) {
                    h->sort.platEnc[code] = gi; /* Supp PS encodings */
                }
            }
        }
    }
    ...
    if (!IS_CID(g)) {
        ...
        for (i = 0; i < nGlyphs; i++) {
            const char *srcName;
            hotGlyphInfo &gi = g->glyphs[i];
            srcName = g->goadb->getSrcGlyphName(gi.gname.c_str());
            if (strcmp(srcName, gi.gname.c_str()) != 0) {
                gi.srcName = srcName;
            }
        }
    }
...
void mapReuse(hotCtx g) {
    ...
    for (i = 0; i < h->sort.glyphAddlUV.cnt; i++) {
        g->glyphs[h->sort.glyphAddlUV.array[i]].addlUV.clear();
    }
