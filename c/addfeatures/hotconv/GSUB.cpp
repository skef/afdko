...
void GSUBNew(hotCtx g) {
    GSUBCtx h = (GSUBCtx)MEM_NEW(g, sizeof(struct GSUBCtx_));

    h->nw.script = h->nw.language = h->nw.feature = TAG_UNDEF;

    dnaINIT(g->DnaCTX, h->nw.rules, 50, 200);
...
void GSUBFeatureBegin(hotCtx g, Tag script, Tag language, Tag feature) {
    ...
    if (h->nw.script != script) {
        h->nw.script = script;
    }
    if (h->nw.language != language) {
        h->nw.language = language;
    }
    if (h->nw.feature != feature) {
        h->nw.feature = feature;
    }
}

static void startNewSubtable(hotCtx g) {
    GSUBCtx h = g->ctx.GSUB;
    Subtable *sub = h->nw.sub = dnaNEXT(h->subtables);
    int hasFeatureParam = ((h->nw.lkpType == GSUBFeatureNameParam) ||
                           (h->nw.lkpType == GSUBCVParam));

    sub->script = h->nw.script;
    sub->language = h->nw.language;
    sub->feature = h->nw.feature;
    STRCPY_S(sub->id_text, sizeof(sub->id_text), g->error_id_text);  // save feature and lookup names for writing phase
    sub->lkpType = h->nw.lkpType;
    sub->lkpFlag = h->nw.lkpFlag;
    sub->markSetIndex = h->nw.markSetIndex;
    sub->label = h->nw.label;
    if (hasFeatureParam) {
...
void GSUBLookupBegin(hotCtx g, unsigned lkpType, unsigned lkpFlag, Label label,
    ...
    h->nw.useExtension = useExtension;
    h->nw.lkpType = lkpType;
    h->nw.lkpFlag = lkpFlag;
    h->nw.markSetIndex = useMarkSetIndex;
    h->nw.label = label;
...
void GSUBLookupEnd(hotCtx g, Tag feature) {
    ...
    switch (h->nw.lkpType) {
        ...
        case GSUBFeatureNameParam:
            fillGSUBFeatureNameParam(g, h, h->nw.sub);
            break;

        case GSUBCVParam:
            fillGSUBCVParam(g, h, h->nw.sub);
            break;

        default:
            /* Can't get here, but it is a useful check for future development. */
            hotMsg(g, hotFATAL, "unknown GSUB lkpType <%d> in %s.", h->nw.lkpType, g->error_id_text);
    }
...
void GSUBAddFeatureMenuParam(hotCtx g, void *param) {
    ...
    feat_param = (FeatureNameParameterFormat *)MEM_NEW(g, param_size);
...
static int CTL_CDECL cmpSingle(const void *first, const void *second) {
...
static void checkAndSortSingle(hotCtx g, GSUBCtx h) {
    ...
    qsort(h->nw.rules.array, h->nw.rules.cnt, sizeof(SubstRule), cmpSingle);

    for (i = 1; i < h->nw.rules.cnt; i++) {
        SubstRule *curr = &h->nw.rules.array[i];
    ...
    if (nDuplicates > 0) {
        ...
        qsort(h->nw.rules.array, h->nw.rules.cnt, sizeof(SubstRule),
              cmpSingle);
        h->nw.rules.cnt -= nDuplicates;
    }
}
...
static void fillSingle2(hotCtx g, GSUBCtx h) {
    ...
    fmt->SubstFormat = 2;
    fmt->Coverage = fillSingleCoverage(g, h, otl); /* Adjusted later */
    fmt->GlyphCount = (unsigned short)h->nw.rules.cnt;
    fmt->Substitute = (GID *)MEM_NEW(g, sizeof(GID) * fmt->GlyphCount);
    for (i = 0; i < fmt->GlyphCount; i++) {
        fmt->Substitute[i] = h->nw.rules.array[i].repl->gid;
    }
...
static void writeSingle1(hotCtx g, GSUBCtx h, Subtable *sub) {
    SingleSubstFormat1 *fmt = (SingleSubstFormat1 *)sub->tbl;
...
static void writeSingle2(hotCtx g, GSUBCtx h, Subtable *sub) {
    int i;
    SingleSubstFormat2 *fmt = (SingleSubstFormat2 *)sub->tbl;
...
static int CTL_CDECL cmpMultiple(const void *first, const void *second) {
...
static void fillMultiple1(hotCtx g, GSUBCtx h, long beg, long end, long size,
    ...
    long i;
    Offset offset;
    MultipleSubstFormat1 *fmt =
        (MultipleSubstFormat1 *)MEM_NEW(g, sizeof(MultipleSubstFormat1));
#define AALT_STATS 1 /* Print aalt statistics with debug output */

#if HOT_DEBUG
    if (beg != 0 || end != h->nw.rules.cnt - 1) {
    ...
    fmt->sequence = (Offset *)MEM_NEW(g, sizeof(Offset) * nSequences);
    fmt->sequence_ = (Sequence *)MEM_NEW(g, sizeof(Sequence) * nSequences);
...
static void fillMultiple(hotCtx g, GSUBCtx h) {
    ...
    aaltDump(g, h);
#endif
    for (j = 0; j < h->nw.rules.cnt; j++) {
        GNode *node;
        unsigned nSubsNew;
        long sizeNew;
        SubstRule *rule = &h->nw.rules.array[j];
        ...
        nSubsNew = nSubs;
        for (node = h->nw.rules.array[j].repl; node != NULL;
             node = node->nextSeq) {
            nSubsNew++;
        }
...
static void writeMultiple(hotCtx g, GSUBCtx h, Subtable *sub) {
    long i;
    MultipleSubstFormat1 *fmt = (MultipleSubstFormat1 *)sub->tbl;
...
static int CTL_CDECL cmpAlternate(const void *first, const void *second) {
    GID a = ((SubstRule *)first)->targ->gid;
    GID b = ((SubstRule *)second)->targ->gid;
...
static void fillAlternate1(hotCtx g, GSUBCtx h, long beg, long end, long size,
    ...
    long i;
    Offset offset;
    AlternateSubstFormat1 *fmt =
        (AlternateSubstFormat1 *)MEM_NEW(g, sizeof(AlternateSubstFormat1));
#define AALT_STATS 1 /* Print aalt statistics with debug output */
    ...
    fmt->alternateSet = (Offset *)MEM_NEW(g, sizeof(Offset) * nAltSets);
    fmt->alternateSet_ = (AlternateSet *)MEM_NEW(g, sizeof(AlternateSet) * nAltSets);
...
static void writeAlternate(hotCtx g, GSUBCtx h, Subtable *sub) {
    long i;
    AlternateSubstFormat1 *fmt = (AlternateSubstFormat1 *)sub->tbl;
...
static int CTL_CDECL cmpLigature(const void *first, const void *second) {
...
static unsigned totNumComp(GSUBCtx h) {
    ...
    for (i = 0; i < h->nw.rules.cnt; i++) {
        for (node = h->nw.rules.array[i].targ; node != NULL;
             node = node->nextSeq) {
            cnt++;
        }
    }
...
static void fillLigature(hotCtx g, GSUBCtx h) {
    ...
    unsigned nLigSets = 0;
    LigatureSubstFormat1 *fmt =
        (LigatureSubstFormat1 *)MEM_NEW(g, sizeof(LigatureSubstFormat1));
    ...
    fmt->ligatureSet = (Offset *)MEM_NEW(g, sizeof(Offset) * nLigSets);
    fmt->ligatureSet_ = (LigatureSet *)MEM_NEW(g, sizeof(LigatureSet) * nLigSets);

    iLigSet = 0;
    j = 0;
    for (i = 1; i <= h->nw.rules.cnt; i++) {
        if (i == h->nw.rules.cnt || h->nw.rules.array[i].targ->gid !=
                                         (h->nw.rules.array[i - 1]).targ->gid) {
            /* --- Fill a LigatureSet --- */
            int k;
            long iLast;
            LOffset offLig;
            ...
            if (j == 0) {
                ligSet->ligature =
                    (Offset *)MEM_NEW(g, sizeof(Offset) * h->nw.rules.cnt);
                ligSet->ligature_ =
                    (Ligature *)MEM_NEW(g, sizeof(Ligature) * h->nw.rules.cnt);
            } else {
            ...
            for (k = iLigSet; k <= iLast; k++) {
                /* --- Fill a Ligature --- */
                GNode *node;
                SubstRule *ligRule = &h->nw.rules.array[k];
                ...
                if (j == 0 && k == 0) {
                    lig->Component = pComp =
                        (GID *)MEM_NEW(g, (totNumComp(h) - h->nw.rules.cnt) *
                                           sizeof(GID));
                } else {
                    lig->Component = pComp;
                }
...
static void writeLigature(hotCtx g, GSUBCtx h, Subtable *sub) {
    long i;
    LigatureSubstFormat1 *fmt = (LigatureSubstFormat1 *)sub->tbl;
...
static Label addAnonRule(hotCtx g, GSUBCtx h, GNode *pMarked, unsigned nMarked,
    ...
    si->parentFeatTag = h->nw.feature;
    si->useExtension = h->nw.useExtension;
...
static LOffset fillChain1(hotCtx g, GSUBCtx h, long inx) {
    ChainContextSubstFormat1 *fmt =
        (ChainContextSubstFormat1 *)MEM_NEW(g,
                                            sizeof(ChainContextSubstFormat1));
...
static LOffset fillChain2(hotCtx g, GSUBCtx h, long inx) {
    ChainContextSubstFormat2 *fmt =
        (ChainContextSubstFormat2 *)MEM_NEW(g,
                                            sizeof(ChainContextSubstFormat2));
...
static void fillChain3(hotCtx g, GSUBCtx h, otlTbl otl, Subtable *sub,
    ...
    SubstRule *rule = &h->nw.rules.array[inx];
    unsigned nSubst = rule->repl != NULL; /* Limited support */

    ChainContextSubstFormat3 *fmt =
        (ChainContextSubstFormat3 *)MEM_NEW(g,
                                            sizeof(ChainContextSubstFormat3));
    ...
    if (nSubst == 0) {
        ...
        if (rule->repl != NULL) {
            ...
            fmt->substLookupRecord =
                (SubstLookupRecord *)MEM_NEW(g, sizeof(SubstLookupRecord) *
                                                nSubst);
            ...
            fmt->substLookupRecord->LookupListIndex =
                addAnonRule(g, h, pMarked, nMarked, rule->repl, h->nw.lkpFlag, h->nw.markSetIndex);
...
static void writeChain3(hotCtx g, GSUBCtx h, Subtable *sub) {
    ...
    ChainContextSubstFormat3 *fmt = (ChainContextSubstFormat3 *)sub->tbl;
    int isExt = sub->extension.use;
...
static void freeChain1(hotCtx g, GSUBCtx h, Subtable *sub) {
    ChainContextSubstFormat1 *fmt = (ChainContextSubstFormat1 *)sub->tbl;
...
static void freeChain2(hotCtx g, GSUBCtx h, Subtable *sub) {
    ChainContextSubstFormat2 *fmt = (ChainContextSubstFormat2 *)sub->tbl;
...
static int CTL_CDECL cmpNode(const void *first, const void *second) {
...
/* Compare glyph ids */
#if 0
static int CTL_CDECL cmpGlyphIds(const void *first, const void *second) {
    GID a = *(GID *)first;
    GID b = *(GID *)second;
    if (a < b) {
        return -1;
...
static void fillReverseChain1(hotCtx g, GSUBCtx h, otlTbl otl, Subtable *sub,
    ...
    ReverseChainContextSubstFormat1 *fmt = (ReverseChainContextSubstFormat1 *)
    ...
    if ((subCount == 0) || (rule->repl == NULL)) {
        ...
        fmt->Substitute = (GID *)MEM_NEW(g, sizeof(GID) * subCount);
        i = 0;
        for (p = pInput; p != NULL; p = p->nextCl) {
...
static void writeReverseChain(hotCtx g, GSUBCtx h, Subtable *sub) {
    ...
    ReverseChainContextSubstFormat1 *fmt =
        (ReverseChainContextSubstFormat1 *)sub->tbl;
    int isExt = sub->extension.use;
...
static ExtensionSubstFormat1 *fillExtension(hotCtx g, GSUBCtx h,
                                            unsigned ExtensionLookupType) {
    ExtensionSubstFormat1 *fmt =
        (ExtensionSubstFormat1 *)MEM_NEW(g, sizeof(ExtensionSubstFormat1));
