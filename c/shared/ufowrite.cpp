...
static void writeGlyphOrder(ufwCtx h) {
    ...
    snprintf(buffer, sizeof(buffer), "%s", "lib.plist");
    h->cb.stm.clientFileName = buffer;
    h->stm.dst = h->cb.stm.open(&h->cb.stm, UFW_DST_STREAM_ID, 0);
    if (h->stm.dst == NULL)
        fatal(h, ufwErrDstStream);
    ...
    if (h->top->sup.flags & ABF_CID_FONT) {
        if (h->top->cid.CIDFontName.ptr != NULL) {
            ...
            snprintf(buffer, sizeof(buffer), "\t<string>%s</string>",
                     h->top->cid.Registry.ptr);
            writeLine(h, buffer);
            writeLine(h, "\t<key>com.adobe.type.cid.Ordering</key>");
            snprintf(buffer, sizeof(buffer), "\t<string>%s</string>",
                     h->top->cid.Ordering.ptr);
            writeLine(h, buffer);
            writeLine(h, "\t<key>com.adobe.type.cid.Supplement</key>");
            snprintf(buffer, sizeof(buffer), "\t<integer>%ld</integer>",
                     h->top->cid.Supplement);
            writeLine(h, buffer);
        }
    }
...
static void writeMetaInfo(ufwCtx h) {
    ...
    snprintf(buffer, sizeof(buffer), "%s", "metainfo.plist");
    h->cb.stm.clientFileName = buffer;
    h->stm.dst = h->cb.stm.open(&h->cb.stm, UFW_DST_STREAM_ID, 0);
    if (h->stm.dst == NULL)
        fatal(h, ufwErrDstStream);
...
static int writeFontInfo(ufwCtx h, abfTopDict *top) {
    ...
    if (top->sup.flags & ABF_CID_FONT) {
        ...
        snprintf(buffer, sizeof(buffer),
                 "\t<integer>%d</integer>", (int)h->top->FSType);
        writeLine(h, buffer);
        ...
        for (j = 0; j < top->FDArray.cnt; j++) {
            ...
            snprintf(buffer, sizeof(buffer),
                     "\t<string>%s</string>", fd->FontName.ptr);
            writeLine(h, buffer);
            writeLine(h, "\t<key>PaintType</key>");
            snprintf(buffer, sizeof(buffer),
                     "\t<integer>%ld</integer>", fd->PaintType);
            writeLine(h, buffer);
