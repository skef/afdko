...
static const uint8_t hexmap[] = "0123456789ABCDEF";
...
static void hexEncrypt(t1wCtx h, size_t cnt, const uint8_t *p, int newline) {
    ...
    if (newline) {
        /* Add newline */
        for (p = (const uint8_t *)h->arg.newline; *p != '\0'; p++)
            *q++ = *p;
        h->eexec.cnt = 0;
    }
...
static void writeReal(t1wCtx h, float value) {
    ...
    if (roundf(value) == value)
        snprintf(buf, sizeof(buf), "%ld", (long)roundf(value));
    else {
...
static void writeLine(t1wCtx h, const char *s) {
    writeStr(h, s);
...
static void CTL_CDECL writeFmt(t1wCtx h, const char *fmt, ...) {
    char buf[200];
    va_list ap;
    va_start(ap, fmt);
...
static int writeTmp(t1wCtx h, long cnt, const uint8_t *ptr) {
    return h->cb.stm.write(&h->cb.stm, h->stm.tmp, cnt,
                           (const char *)ptr) != (size_t)cnt;
}

/* Encrypt charstring and write to tmp stream. */
static int saveCstr(t1wCtx h, abfGlyphInfo *info,
                    long length, uint8_t *data, Cstr *cstr) {
    /* Encrypted priming bytes */
    static const uint8_t primer[] = {0x1c, 0x60, 0xd8, 0xa8};
    long i;
    uint8_t *p;
    ...
    if (info != NULL && info->flags & ABF_GLYPH_CID &&
        ...
        if (writeTmp(h, 1, (const uint8_t *)&info->iFD))
            return 1;
        cstr->length++;
    }
    ...
    switch (h->arg.lenIV) {
        ...
        case 0:
            ...
            break;
        case 1:
            if (writeTmp(h, 1, (const uint8_t *)primer))
                return 1;
            r = 27725;
            cstr->length += 1;
            break;
        case 4:
            if (writeTmp(h, 4, primer))
                return 1;
            r = 17114;
            cstr->length += 4;
            break;
...
/* Save subr. */
static long saveSubr(t1wCtx h, size_t length, uint8_t *data) {
    Cstr *cstr;
...
static int strNeedsEscape(const char *str) {
    const char *p;
    int state = 0;
    for (p = str; *p != '\0'; p++)
        switch (*p) {
...
static void writeEscapedStr(t1wCtx h, const char *str) {
    const char *p;
    for (p = str; *p != '\0'; p++)
        switch (*p) {
            case '(':
...
static void writeStringDef(t1wCtx h, const char *key, const char *value) {
...
static void writeLiteralDef(t1wCtx h, const char *key, const char *value) {
    if (value == ABF_UNSET_PTR)
        return;
    writeFmt(h, "/%s /", key);
    writeStr(h, value);
...
static void writeBooleanDef(t1wCtx h, const char *key, int value) {
    writeFmt(h, "/%s %s def%s", key, value ? "true" : "false", h->arg.newline);
}

/* Write PostScript definition of integer object. */
static void writeIntDef(t1wCtx h, const char *key, long value) {
...
static void writeRealDef(t1wCtx h, const char *key, float value) {
    char buf[50];
...
/* Write PostScript Version of real object. */
static void writeVersionDef(t1wCtx h, const char *key, float value) {
    if (value == ABF_UNSET_REAL)
        return;
...
static void writeIntArrayDef(t1wCtx h, const char *key, long cnt,
                             long *array) {
    const char *sep;
...
static void writeRealArrayDef(t1wCtx h, const char *key, long cnt,
                              float *array) {
    const char *sep;
    if (cnt == ABF_EMPTY_ARRAY)
        return;
...
static int getStdEnc(t1wCtx h, const char *gname) {
...
static void writeTextArray(t1wCtx h, int cnt, const char *array[]) {
    int i;
    for (i = 0; i < cnt; i++) {
        const char *beg = array[i];
        do {
...
static void writeOtherSubrs(t1wCtx h, abfPrivateDict *priv) {
    static const char *hintothers[] = {
#include "t1write_hintothers.h"
    };
    static const char *flexothers[] = {
#include "t1write_flexothers.h"
    };
    static const char *gcothers[] = {
#include "t1write_gcothers.h"
    };
    static const char *procsetothers[] = {
...
static void writePrivateDict(t1wCtx h, abfPrivateDict *priv, long SDBytes) {
    ...
    int size =
        (1 /* BlueValues */ +
         ...
         (priv->LanguageGroup != cff_DFLT_LanguageGroup) * 2 +
         (priv->ExpansionFactor != (float)cff_DFLT_ExpansionFactor) +
         (priv->initialRandomSeed != (float)cff_DFLT_initialRandomSeed) +
         1 /* password */ +
         (h->arg.lenIV != 4) +
         1 /* MinFeature */ +
         1 /* OtherSubrs */);
    ...
    if (priv->BlueValues.cnt == ABF_EMPTY_ARRAY)
        ...
        writeRealArrayDef(h, "BlueValues", priv->BlueValues.cnt,
                          priv->BlueValues.array);
    writeRealArrayDef(h, "OtherBlues", priv->OtherBlues.cnt,
                      priv->OtherBlues.array);
    writeRealArrayDef(h, "FamilyBlues", priv->FamilyBlues.cnt,
                      priv->FamilyBlues.array);
    writeRealArrayDef(h, "FamilyOtherBlues", priv->FamilyOtherBlues.cnt,
                      priv->FamilyOtherBlues.array);
    if (priv->BlueScale != (float)cff_DFLT_BlueScale)
        writeRealDef(h, "BlueScale", priv->BlueScale);
    if (priv->BlueShift != cff_DFLT_BlueShift)
        writeRealDef(h, "BlueShift", priv->BlueShift);
    if (priv->BlueFuzz != cff_DFLT_BlueFuzz)
        writeRealDef(h, "BlueFuzz", priv->BlueFuzz);
    if (priv->StdHW != ABF_UNSET_REAL)
        writeRealArrayDef(h, "StdHW", 1, &priv->StdHW);
    if (priv->StdVW != ABF_UNSET_REAL)
        writeRealArrayDef(h, "StdVW", 1, &priv->StdVW);
    ...
    if (priv->ForceBold != cff_DFLT_ForceBold)
        writeBooleanDef(h, "ForceBold", priv->ForceBold);
    if (priv->LanguageGroup != cff_DFLT_LanguageGroup) {
        writeIntDef(h, "LanguageGroup", priv->LanguageGroup);
        writeLine(h, "/RndStemUp false def");
    }
    if (priv->ExpansionFactor != (float)cff_DFLT_ExpansionFactor)
        writeRealDef(h, "ExpansionFactor", priv->ExpansionFactor);
    if (priv->initialRandomSeed != (float)cff_DFLT_initialRandomSeed)
        writeRealDef(h, "initialRandomSeed", priv->initialRandomSeed);
...
static void writeIdentComment(t1wCtx h) {
    ...
    writeFmt(h, "%%ADOt1write: (%s)%s",
             CTL_SPLIT_VERSION(version_buf, sizeof(version_buf), T1W_VERSION),
             h->arg.newline);
...
static void writeAddnEncoding(t1wCtx h, const char *FontName) {
    long i;
    int seenEnc = 0;
    for (i = 0; i < h->glyphs.cnt; i++) {
        abfGlyphInfo *info = h->glyphs.array[i].info;
...
static void writeAddnNameKeyedFont(t1wCtx h) {
    const char *FontName = h->top->FDArray.array[0].FontName.ptr;
    writeIdentComment(h);
    writeLine(h, "systemdict begin");
    if (h->arg.flags & T1W_DONT_LOOKUP_BASE)
        writeStr(h, FontName);
...
/* Write FDArray. */
static void writeFDArray(t1wCtx h, long FDCnt, uint8_t *FDMap, long SDBytes) {
    long i;
    long j;
    int host = (h->arg.flags & T1W_TYPE_HOST) != 0;
    ...
    for (i = 0; i < h->top->FDArray.cnt; i++)
        if (!host || FDMap[i]) {
            ...
            if (host)
                FDMap[i] = (uint8_t)j; /* Remap dictionary for CIDMap */
            j++;
        }
    writeLine(h, "def");
}
...
static void writeGlyphDirectory(t1wCtx h) {
    ...
    for (i = 0; i < h->glyphs.cnt; i++) {
        char buf[50];
        Glyph *glyph = &h->glyphs.array[i];
        snprintf(buf, sizeof(buf), "%hu", glyph->info->cid);
        writeStr(h, buf);
...
static void writeStartData(t1wCtx h, uint8_t *FDMap, long StartDataSize,
    ...
    /* Write header */
    snprintf(buf, sizeof(buf), "(Binary) %ld StartData ",
             StartDataSize);
...
static void writeAddnCIDKeyedFont(t1wCtx h) {
    const char *FontName = h->top->cid.CIDFontName.ptr;
    writeIdentComment(h);
...
t1wCtx t1wNew(ctlMemoryCallbacks *mem_cb, ctlStreamCallbacks *stm_cb,
    ...
    /* Allocate context */
    h = (t1wCtx) mem_cb->manage(mem_cb, NULL, sizeof(struct t1wCtx_));
    if (h == NULL)
        return NULL;
...
static void saveOp(t1wCtx h, int op) {
    ...
    h->cstr.array[h->cstr.cnt++] = (uint8_t)op;
}

/* Save integer in charstring. */
static void saveInt(t1wCtx h, long i) {
    uint8_t *t = (uint8_t *)&h->cstr.array[h->cstr.cnt];
...
static void dbt1cstr(long length, uint8_t *cstr) {
