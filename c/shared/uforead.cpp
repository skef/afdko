...
static bool keyValueParseable(ufoCtx h, xmlNodePtr cur, const char* keyValue,
    ...
    if (keyValue == NULL) {
        if (!h->parseState.parsingValueArray)
            valid = false;
//            h->logger->log(sWARNING, "Encountered missing value for fontinfo key %s. Skipping", keyName);
...
static void addGLIFRec(ufoCtx h, const char* glyphName, xmlNodePtr cur) {
    ...
    newGLIFRec->glifFileName = (char *)memNew(h, l+1);
    snprintf(newGLIFRec->glifFileName, l+1, "%s", fileName);
    newGLIFRec->altLayerGlifFileName = NULL;
}

static int findGLIFRecByName(ufoCtx h, const char *glyphName) {
    int i = 0;
...
static void updateGLIFRec(ufoCtx h, const char* glyphName, xmlNodePtr cur) {
    ...
    if (index == -1) {
        h->logger->log(sWARNING, "Glyph '%s' is in the processed layer but not in the default layer.", glyphName);
    } else {
        GLIF_Rec* glifRec;
        ...
        int l = strlen(fileName);
        glifRec->altLayerGlifFileName = (char *)memNew(h, l+1);
        snprintf(glifRec->altLayerGlifFileName, l+1, "%s", fileName);
    }
}
...
static void addCharFromGLIF(ufoCtx h, int tag, const char* glyphName,
                            long char_begin, long char_end, unsigned long unicode) {
    abfGlyphInfo* chr;
